...
int main(int argc, char **argv) {
	...
	long unsigned int seed=46573523122;//11145792458;//956475;
	...
	slotBounds bounds1(1,1.45);//slotBounds bounds1(-1,-0.5);
	slotBounds bounds2(1.45,1.9);//slotBounds bounds2(-0.5,0);
	slotBounds bounds3(1.9,2.8);//slotBounds bounds3(0, 0.5);
	...
	pair<double,double> total[numSteps][3]; for(int i=0; i<numSteps;i++) for(int j=0;j<3;j++) total[i][j]=make_pair(0,0);
	
	ofstream output("histogram_testoutput.dat");
	
	int maxRounds=100;
	int samplingStepsCounter[maxRounds+2];
	for(int round=0;round<maxRounds+2;round++)
		{
	...
	for(int i=0; i<samplingSteps+samplingSteps/maxRounds;i++)
		{
		bool accept=false;
		while(accept==false)
			{
		...
		for(int round=0;round<maxRounds+2;round++)		//last entry is total histogram with everything
			{
			if( (i<round*samplingSteps/maxRounds) || (i>=(round+1)*samplingSteps/maxRounds))
				{
	...
	for(int round=0;round<maxRounds+2;round++)
		...
		if( acceptance && round<maxRounds+1) numberOfGoodFits++;
		...
		for(int i=0; i<numSteps;i++) 
			...
			if( acceptance && round<maxRounds+1)
				{
				...
				total[i][1].first+=testcurvature.splineValue(currentVar); total[i][1].second+=testcurvature.splineValue(currentVar)*testcurvature.splineValue(currentVar);
				total[i][2].first+=basisResult.first; total[i][2].second+=basisResult.first*basisResult.first;
				}
	...
	for(int i=0; i<numSteps;i++) 
		...
		for(int j=0; j<3;j++) 
			...
			total[i][j].second=sqrt((numberOfGoodFits-1)*total[i][j].second);
